...
namespace DokanNetMirror
{
    ...
    internal class UnsafeMirror : Mirror, IDokanOperationsUnsafe
    {
        ...
        public UnsafeMirror(ILogger logger, IEnumerable<FolderMap> folderMaps) : base(logger, folderMaps) { }
        ...
        public NtStatus WriteFile(string fileName, IntPtr buffer, uint bufferLength, out int bytesWritten, long offset, IDokanFileInfo info)
        {
            ...
            {
                using (var stream = new FileStream(GetPath(fileName, info.IsDirectory)[0], FileMode.Open, System.IO.FileAccess.Write))
                {
